...
export async function finishD1(env: Env, accountId: string, clerkIds: string[], deviceIds: string[], nowMs: number): Promise<void> {
  ...
  const stmts: D1PreparedStatement[] = [
    ...
    data.prepare("DELETE FROM fairuse_group_progress WHERE account_id = ?").bind(a),
    data.prepare("DELETE FROM fairuse_workspace_group_totals WHERE account_id = ?").bind(a),
