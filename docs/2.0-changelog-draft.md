...
- **Upgrading from 1.x works, and rbox repairs itself along the way.** Run the
  ...
  - The sync wire format itself did **not** change: manifest schema is still
    4, as it was in 1.11.4. What changed is how each machine stores its own
    local record of what it has seen.
...
- **RboxBar ships with every release.** The macOS menu bar app is published
  alongside the CLI binaries for each tag, so it no longer has to be built by
  hand. It is ad-hoc signed rather than notarized, so the first launch needs a
  right-click → Open.
...
- **Leftover files from finished Git operations no longer strand other
  ...
  merge messages are recognised as finished; genuinely in-progress merges still
  wait, and linked-worktree problems now name the responsible worktree.
