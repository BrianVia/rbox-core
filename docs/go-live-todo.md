...
- [x] **CI/CD production deploys** (originally push-to-`main`; split on 2026-07-16 so green, dev-verified `main` is explicitly promoted to `production`):
  - `.github/workflows/deploy-api.yml` — `production` pushes touching `apps/api/**` run typecheck + API tests, then prod D1 migrations → deploy `rbox-prod-api` / `api.rbox.to` → version upload. The production Workers Builds integration is disconnected.
  - Cloudflare Workers Builds (DEV) — watches `main` and automatically runs the dev D1 migration, deploy, and version-upload commands after every merge.
  ...
  - Repo secret **`CLOUDFLARE_DEPLOY_TOKEN`** (Workers Scripts:Edit + D1:Edit + Cloudflare Pages:Edit + Account:Read), distinct from release.yml's R2-only `CLOUDFLARE_API_TOKEN`. Dev-first workflow documented in `AGENTS.md`.
...
2. ✅ **DONE — web↔CLI account link** (doc 21, shipped PR #2): `rbox account link` binds the Clerk identity onto the real account; `rbox subscribe` pays directly onto it.
...
4. ✅ **DONE — credential-kind route gating** (shipped with doc 21): `Principal.kind` + a default-deny route policy 403s web-session tokens on every durable-mint / crypto / sync route.

- [x] `docs/design/16-new-device-emails.md` — **BUILT** (outbox + Queue + cron backstop + CF Email Service send; see doc header) — "new device added" security email. Outbound has **no Cloudflare-native path** (Email Workers only send to verified destinations). **DECISION 2026-06-29: stay all-Cloudflare → MailChannels Email API (paid)** on `security.rbox.to` (overrides the doc's Resend recommendation; provider-specific SPF/DKIM swapped at build time). Durable D1 outbox + Queues (no `ctx.waitUntil` in the worker's `fetch`).
...
- [x] `docs/design/18-support-email-routing.md` — **SHIPPED 2026-06-30.** `support@`/`postmaster@`/`security@rbox.to` → `brian.a.via@gmail.com` via **Cloudflare Email Routing**; migrated off Namecheap (eforward MX removed), apex SPF → Cloudflare, catch-all = **Drop**, DMARC unchanged. Verified end-to-end.
...
- [x] `docs/design/21-account-linking.md` — **SHIPPED 2026-06-30 (PR #2, prod).** web↔CLI account link (`rbox account link`) + token-kind route gate + `rbox subscribe` (primary billing) + re-point saga (fallback). Migration `0014` applied to prod. Closes prereqs #2 + #4.
