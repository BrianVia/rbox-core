...
**Status:** ✅ IMPLEMENTED & VERIFIED. 3 security-review rounds → READY. Verified live: **16/16 cross-tenant isolation** (account B cannot GET A's blobs, read A's workspace/versions/manifests, commit to A's ws — all 404; cannot claim A's blob by reference — 422; platform-only GC; account-scoped device list) + **5/5 happy-path** (login → link creates owned ws → cross-dir same-account sync). antislop: no new duplication (shared authz.ts/util.ts). v2 design below (post review #1, 4 Criticals). The entitlement primitive is kept but made non-bypassable; ownership moves to workspace-creation time; every workspace/blob route is gated; auth becomes role-aware. Pending review #2.
...
7. **Platform vs tenant authz [review#2 Critical].** `POST /v1/admin/gc` requires a **platform** principal — a `RBOX_PLATFORM_SECRET` Wrangler secret (`X-Rbox-Platform` header), NOT a tenant device token. `roots`/`prune` are **internal-only**: the public Worker router does NOT forward them to the DO; they're reachable solely via GC's direct DO binding fetch. (Confirmed: the Worker forwards only `manifests|latest|connect|manifests/:seq`.)
8. **Account-scoped multipart [review#2 High].** `uploads` gains `account_id`; `multipart init` records it; `part`/`complete` verify the caller's account owns the upload (`(account_id, sha, upload_id)`), so a leaked `uploadId` can't let B complete A's upload and mint `blob_refs(B, sha)` without possessing bytes.
9. **Fail-closed ownership [review#2]:** no D1 owner row for a ws → 404, and the DO does NOT bootstrap-cache an owner from an unowned ws. Membership add/join is owner/admin-only and same-account (M7).
- (Stale v1 text about first-commit ownership / 403-cross-account is superseded by the resolutions above — creation-time ownership + 404-everywhere are authoritative.)
