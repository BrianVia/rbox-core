...
_Last updated: 2026-07-18 (marathon session close — storage fiasco solved end-to-end, three releases, two design cycles) — **v1.7.0 + v1.7.1 FLEET-LIVE (v1.7.2 via codex CI thread; fleet on 1.7.2)** — v1.7.0: 138 reset hardening + 141 git-shapes burn-in (#308, 494 PASS/0 FAIL after 8 live runs, engine gaps `bisect-invisible`/`rebase-post-abort-epipe` pinned as findings) + RboxBar Raycast fix; v1.7.1: **design-84 snapshot manifests DEFAULT-ON** (PR #309; device inventory verified ≥v1.1.0 Phase-B floor; `RBOX_MDE_SNAPSHOT=0` kill switch). **STORAGE FIASCO SOLVED**: measurement completed (partition exact 940,446/940,446) → 254GiB = 4.3 active + ~181GiB MANIFESTS (4,344×~40MB ≈ 4,780 sequences; d84 fix existed since Jul 12, flag-gated OFF) + ~10GiB refset sidecars + 57GiB stranded; NOT bundles (incrementals were on), NOT dedup (convergent HKDF healthy). Manual purge via existing admin routes (`x-rbox-platform` header; plan-flip→retention→`gc?phase=phase1&graceMs=0` loops; ~470 calls at 2,000 rows/tick) → **account at 4.3GiB, exactly the measured head set**. Bugs found on the way: `adminSetPlan(none)` stamps `grace_until`, restore doesn't clear it, and `retentionPrune` skips ANY grace-stamped account (code contradicts comment) — fix designed into 149; R2 list pagination missing-`result_info` crash fixed on main. **DESIGN 149 (storage economics) ALIGNED after 5 adversarial rounds, merged #313**: capability-gated envelope defaults (replaces v1.7.1 manual gate; snapshot+delta graduate together per founder default-on ruling), refset sidecar deltas (two signed identities, server fold, per-sequence chain rooting), fair-use 5× floor-adjusted enforcement (exact marginal-release selector, one-move-per-epoch, bounded-overshoot semantics) + grace read fix; **step-1 implementation dispatched** (0029 migration, observe-only scan ledger). **ARCHITECTURE AUDIT adopted** (`docs/audits/2026-07-17-product-core-architecture-audit.md`, codex, 24 findings — headline: canonical GC has a HARD 8-workspace-row fleet-wide ceiling, dormant at 2 rows, counts down with user growth): Phase 0 = **design 151** (GC observability, cappedJson bounds, fail-closed untrack/kill consent, atomic credentials) in review (r1: 17 findings incl. upgrade-treats-hung-daemon-as-restarted; v2 deriving). **Queue after**: 149 steps 2–4 → 151 impl → **152 publish-engine selection (founder don't-forget: rig-qualify d98 vs legacy, winner default-on, loser DELETED)** → audit phases 1–4 (incl. structural cycle: apply.ts split + 15 unproven invariants) → 139 plain-words, git-shapes follow-ups, R2 physical cleanup (~75GiB condemned, our bill, needs pack-gc execute). **New standing rules**: founder default-on preference (new behavior ships ON with kill switch; stage only wire-compat/breaking/named-bake-condition — memory `default-on-preference.md`); design-number ledger (150 = codex CI thread — that surface is theirs, core product is this session's); GC admin escape hatches proven (playbook: plan-flip+grace-clear+phase1-loop drains an account in ~1h). Worktrees swept: 35 removed (PR-verified or bundle-archived), `product-core-audit` kept (Brian's WIP)._

_Prior header (2026-07-17 day): **DESIGN 138 MERGED (#306) + 142 PHASE-0 MERGED (#305, +143/144 fixes #307) + v1.6.9 FLEET-LIVE + STORAGE INCIDENT ARC CLOSED.** **138 reset-path hardening MERGED**: 9 adversarial rounds (convergence 10→9→8→5→4→2→1), NORMATIVE crash-window row table (codex-derived, ratified), unforgeable consent witness threading 137's wizard seam, repository-order lock fence, daemon halt/heal, crash-resumable quarantine + `rbox doctor reset-journal`; scrutiny's 4 safety hunts CLEAN, 2 composition MAJORs found+fixed (quarantine→rebind dead-end; observation-error poisoning); the derivation also found 2 power-loss fsync bugs. UNRELEASED — rides the next CLI release (v1.7.0 candidate; Max-window rule applies). **STORAGE ARC**: founder acct hit 250/250GiB ⛔ → +100GiB bump (350 cap), fleet-wide respect-gitignore flip, 95,324 legacy paths purged (desktop) + 12 (Mac second pass) — the scary 32k "resurrection" was the Mac's design-44 changed-file guard correctly deferring deletions mid-build, self-drained; usage ~253GiB is now RETAINED HISTORY (365d) + unknown leak share — **142 Phase-0 storage-truth runner MERGED** (read-only decomposition: partition + Phase-1 health probe + R2 reconciliation; scrutiny 8 findings → SHIP; wrangler remote-bindings hang → REST adapter #307; timestamp-gaps degradation for the best-effort commits mirror) — **live founder measurement IN FLIGHT** (route promoted to prod ×2 today). Path down: measure → reclaim stranded/drift via existing GC (or cap fix if probe says fail-closed) → 142 Phase 2 active-state quota (founder-ruled: quota = active files, history tiered/fair-use 5×, pricing unchanged, accelerated pruning TABLED, site copy after ship). **141 git-shapes**: ALIGNED v3 (13→6→annex; 637-line code-derived outcomes annex normative), implemented, FOUR live rig runs (fails 29→5): confirmed engine gap `bisect-invisible` (sidecar-recorded, follow-up design owed), partial-clone "hydration" was an observation artifact (retracted); final 5-fail fix round in flight (s5-rebase convergence may be a 2nd genuine gap). **Also**: RboxBar Raycast bug fixed (candidate list omitted ~/.rbox/bin — the installer's own path; uncommitted, PR pending), hourly founder-update cron live, stale cross-session shells audited/killed. Queue: 141 finish → PR; bar-fix PR; STATUS/artifact refresh; structural cycle (apply.ts split + 15 unproven invariants) next after 141; git-shapes Tier-2 + native-mac backend + bisect-gap + s4-hydration follow-ups named; 139 plain-words pass queued. Founder pendings: v1.7.0 timing; 253GiB decision after measurement (Phase-2 vs retention-trim)._
