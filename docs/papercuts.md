...
- 2026-08-21: FOUR jobs in one CI run (PR #801) fast-failed with the CF runner's "GitHub's runner assignment was not observed within 30 seconds" — a pool-side infra glitch, rerun recovered. Also learned: the ENTIRE test-job matrix (typecheck, workers API, regress lanes) moved to CF with #797, not just the 6 src shards — the matrix shares one runs-on. Watch assignment-timeout rate for the Aug-27 pilot verdict; consider reporting upstream (biw#2 thread) if recurrent.
