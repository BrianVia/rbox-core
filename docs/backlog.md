...
- **Multipart uploads are silent in progress reporting (2026-07-06):** transfer progress emits per FILE, so a single multi-GB git bundle upload produces zero activity.json updates for its whole duration — observed live as an 11-minute-silent (but healthy) daemon mid-first-publish, which reads as wedged in `rbox status`. Fix: emit per-part ticks from putBlobMultipart (parts are ~90MiB, so even a 6GB blob updates every few seconds) via the existing TransferProgress detail channel; also consider counting bytes not files for the upload denominator on first publishes.
