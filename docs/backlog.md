...
- **Post-upload pump awaits have no timeout — 110-minute silent wedge (2026-07-07 ~00:18Z, live):** after the final multipart completed, the daemon sat 110 min at 0% CPU, ZERO open TCP sockets, no activity writes, no log lines — a fetch promise black-holed when its socket died (Bun fetch neither rejected nor timed out; same class as the design-59 bench finding). The redeem/commit/putBlobBytes JSON POSTs need the same protection uploads got in #92: a bounded overall timeout (size-irrelevant, these are small bodies — 60s is generous) + retry under the existing idempotency rules. The activity heartbeat also froze — a secondary fix is letting the heartbeat run on a timer independent of pump progress so `rbox status` can at least say "no progress for Xm" honestly.
- **(RESOLVED 2026-07-07, PR #116 + v0.9.3 — diagnosis corrected)** The "daemon e2ee context stale after roster change" entry described this symptom with the wrong cause: it was the design-72 workspace-config reload rebuilding cfg from workspace.json and dropping the runtime-attached `encrypted`/`kek`/`remoteUrl` (fired on the FIRST safety tick, no roster change needed — v0.9.2's daemons died within minutes universally). Fixed by moving only `respectGitignore` on reload; regression test pins the runtime fields. Kept here as a record because the wrong theory shipped to this file first.
- **GC phase-1 sweep contends with live commits → hourly client-visible 500 (CONFIRMED 2026-07-07):** four 500s tracked the cron exactly — 06:01/08:00/12:00 at the old :00 schedule, then 14:24 after #122 moved the crons to :23. Mechanism: `phase1Mark` SELECTs every `blob_refs` row per account (140k on the founder account) in one read while a commit's accounting txn needs the D1 single writer → transient 500, self-heals next tick. The offset only relocated it. Real fix candidates, cheapest first: (a) paginate the mark-phase read (LIMIT/OFFSET or keyset batches with await yields between), (b) per-account jitter inside the cron handler (spread accounts across the hour), (c) skip accounts whose activity shows an in-flight push (needs a server-visible signal — probably not worth it). All 500s self-heal via the pump retry, so severity = UX noise + wasted cycle, not correctness.
