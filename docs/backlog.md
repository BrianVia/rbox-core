...
- **Daemon's e2ee context goes stale after a roster change and never recovers (2026-07-07 04:08Z, live):** flat-meadow's device admission changed the account roster; the Mac daemon's long-lived pump then failed EVERY push with "E2EE required: refusing to sync without an encryption key" (×10+) while a fresh foreground `rbox sync` worked perfectly and published. The daemon holds its e2ee/key context from startup and has no rebuild path when roster/epoch state moves under it. Fix: on this error class (and on epoch_stale), rebuild the authed remote/key context in-process (same freshness a new CLI invocation gets) instead of erroring forever; a design-45 halt should only record after a rebuild ALSO fails. Workaround: restart the daemon.
