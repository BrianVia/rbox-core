...
src/cli/sync/push.ts          — push retry orchestration (pushManifest) + single-attempt transaction (runPushAttempt) + recovery-page accumulation + stampManifestSchemaForCommit + synchronous keep-mine rider/result and transport-boundary receipt/409/accepted-ACK handling, including caller-required uncached/unpruned scan mode. Never: rendering, remote construction, pull-side apply policy, adoption orchestration, or pre-publication capture-observation persistence (sync-git/git-capture-observation.ts owns it).
