...
src/cli/doctor-machine.ts     — the outside-a-workspace all-workspaces view: enumeration from daemon desired-state rows, per-workspace liveness-gated summary, and its non-exhaustiveness disclosure. Never: per-workspace check collection, network calls, or mutation.
