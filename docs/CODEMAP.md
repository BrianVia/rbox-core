...
src/cli/sync/push.ts          — push retry orchestration (pushManifest) + single-attempt transaction (runPushAttempt) + recovery-page accumulation + stampManifestSchemaForCommit, including caller-required uncached/unpruned scan mode and accepted-ACK-only resolution-intent retirement. Never: rendering, remote construction, pull-side reconcile, or adoption orchestration.
...
src/cli/sync-git/plan.ts             — push planner: planGitSections (§2.7 — one closure unit, helper order is semantic), pending-supersession candidate admission/final-proof flow, keep-mine binding/final-report/pin execution, post-proof conflict-ref hygiene dispatch, plan contracts, push formatters, gitBaseAfterCommit, gitForceForMissingBlobs. Never: apply-side mutation, status rendering.
...
src/cli/sync-git/resolution-intent.ts — design-176 keep-mine binding capture plus preliminary/final directional discard reports and authorization projection. Never: persist/clear intent, publish candidates, mutate refs, or grant BASE authority.
src/cli/sync-git/apply.ts            — pull-side git materialization: applyGitSections, keep-mine P-bound reservation, held-skip orchestration, post-settlement attempt-rebind dispatch, receiver-equivalent repo-key admission, exclusive-leaf apply metrics, config-lane transactions, conflict preservation, quarantine ordering. Never: planning policy.
