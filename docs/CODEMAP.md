...
src/cli/sync/push.ts          — push retry orchestration (pushManifest) + single-attempt transaction (runPushAttempt) + recovery-page accumulation + stampManifestSchemaForCommit + the GitCapturePort and ManifestCommitPort adapters (commit-leased planGitSections, observation write-back, no-op sidecar carry, forensic lines, commit transport + keep-mine arm/disarm + conflict pull/reconcile) + delta/snapshot base selection and the accepted-ACK state transition, including caller-required uncached/unpruned scan mode. Never: rendering, remote construction, pull-side apply policy, adoption orchestration, candidate projection/admission (sync/publish-candidate.ts owns it), commit-response classification (sync/manifest-commit-executor.ts owns it), or pre-publication capture-observation persistence (sync-git/git-capture-observation.ts owns it).
...
src/cli/sync/publish-candidate.ts — sole owner of one sealed publication candidate: forward-ignore/case-fold projection and collision authority, purge-evaluability refusal, files-first genesis admission, the single Git capture effect plan → execution receipt binding, the bounded capture observation it hands to RecordGitCaptureObservation, the zero-commit no-op plus its commit-free sidecar carry, and the push mass-delete breaker. Never: encrypt, upload, POST, BASE advancement, state reads, or Git mutation outside the capture executor.
