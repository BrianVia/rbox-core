...
src/cli/state-plane/store/transition-stages.ts — sealed multi-repository CAS inputs: base-proof admission, per-row source-evidence admission including the global-present rule, and transition row ceilings. Never: authority mutation, BASE composition, the CAS TEMP schema, or global file promotion.
