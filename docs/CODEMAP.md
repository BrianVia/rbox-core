...
src/cli/reset-journal.ts      — design-130 reset-v1 bounded journal validation, candidate/archive/recovery-Z cutover, deterministic crash recovery, and active-Z retirement. Never: ordinary BASE composition, follower authorization, or P-repair policy.
...
src/engine/git/protocol-locks.ts — §130 lock-class ordering/tracing, canonical multi-common-dir operation locks, reflog/origin locks, and the isolated post-HEAD compatibility exception. Never: mutation policy, Git transactions, or state composition.
```
