...
src/cli/state-plane/store/stage-artifacts.ts — id-scoped stage locks, owner-exclusive private directories, no-follow identity-bracketed physical proof, the normative checkpoint/S0/prove/link seal sequence, no-clobber publication, single-accessor contained handles, identity-proven deletion, and always-finalized row streaming. Never: stage schemas, digest grammars, or authority mutation.
...
src/cli/state-plane/store/transition-stages.ts — sealed multi-repository CAS inputs: base-proof and per-row source-evidence admission, transition row ceilings, and the verified TEMP copy. Never: authority mutation, BASE composition, or global file promotion.
