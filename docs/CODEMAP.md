...
src/cli/sync-git/apply.ts            — pull-side git materialization: applyGitSections with uniform pending behavior, held-skip orchestration, receiver-equivalent repo-key admission, exclusive-leaf apply metrics, conflict preservation for present sections, quarantine ordering. Never: keep-mine reservation or planning policy, whole-repo remote-absence reconciliation (sync-git/remote-repository-deletion.ts owns it), received-config lane decisions/transactions (sync-git/received-git-config.ts owns them), the clean-materialization plan/receipt (sync-git/clean-materialization.ts owns it), the standing-P proof/settlement loop (sync-git/standing-branch-proof.ts owns it), the follow proof/BASE/disposition composition (sync-git/follow-repo-transition.ts owns it), or the post-execution commit half — exact-ref revalidation, state-CAS lock planning, unreadable-terminal carry, journal publication, and A/P/K settlement (sync-git/received-git-transition-commit.ts owns them).
src/cli/sync-git/received-git-config.ts — sole owner of the receive-side Git config lane: incoming-config sanitation classification, the sanitize-present/wire-absent baseline transitions, receiver ownership + due disposition, the five-phase `ReceivedGitConfigPlan`, the phase/identity/common-dir-bound `applyReceivedGitConfig` executor contract, and the `ConfigLaneLedger` that owns every local-only lane field write. Never: Git ref/checkout mutation, BASE/pending/deferral lane authority, state persistence, or push-side config capture (sync-git/config-lane.ts owns the capture model).
src/cli/sync-git/clean-materialization.ts — sole owner of the clean/fresh receive transition: ignore/containment/capable-lineage refusals, quarantine-and-ref-wipe authority for a dir leftover at a removed path, the typed A/P/K branch-transition adapter, engine-refusal classification, and one receipt carrying both the physical Git facts and the intended BASE transition, bound plan-to-receipt by an exact repository+incoming-key identity. Never: the received-config predicate (it consumes the settled phase), follower-protocol preparation, sync-state reads, or durable persistence.
...
src/cli/sync-git/follow.ts           — design-116/126 checkout orchestration: artifact/index-equivalence staging guards, all-guard and breadcrumb/content-equivalence-waiver classification, independent safe-ref publication, journaled checkout commit/recovery adapters, and follow leaf-timer plumbing. Never: ORIG_HEAD preservation mechanics, D4a journal/reachability/index/pin mechanics, or state-file persistence.
...
src/cli/daemon/daemon-publish-transition.ts — PublishLocalWorkspaceTransition (`PublishLocalWorkspaceTransition`): sole owner of the daemon-side publish request/outcome reduction — sealing one attempt's LOCAL/BASE/GC-fence/collision/terminal-fingerprint facts into a request, classifying the closed push result (committed / not-committed / terminal block), and reducing it into ONE ordered effect plan the daemon applies verbatim; an outcome bound to another attempt performs no transition. Never: Git planning, candidate projection, commit execution, or publisher acknowledgement (src/cli/sync/'s push spine).
src/cli/daemon/daemon-pull-transition.ts — ApplyRemoteWorkspaceTransition (`ApplyRemoteWorkspaceTransition`): sole owner of the daemon-side pull request/outcome reduction — evaluating design 202/206's trust predicate P into a named view-or-skip, sealing it into a request, classifying the closed pull result (applied / local-untrusted), authorizing at most ONE linked scan-backed re-execute per op, and reducing the result into ONE ordered effect plan (chain-repair clear, propagation, report settle, conflict metric, matcher refresh, post-BASE adoption, F4/F3/F2/F1 refresh choice, provenance log) the daemon applies verbatim; an outcome bound to another attempt performs no transition. Never: reconciliation, apply, the mass-delete breaker, or Git receive policy (src/cli/sync/ + src/cli/sync-git/).
...
src/cli/state-plane/locks.ts — the witness type for the complete held state-plane lock set. Never: lock acquisition, ordering, or protocol logic.
...
src/cli/state-plane/store/write-packet.ts — the CAS driver: pairing and sealed-header admission, one-at-a-time stage consumption, transaction control, rejection/retry and busy outcomes, id-scoped post-adoption cleanup, and the telemetry-binding singleton. Never: stage construction, in-transaction step bodies, or lock ownership policy.
...
src/cli/state-plane/adapters/sqlite-state-save.ts — whole-state save-packet staging into the native sealed global/transition CAS and exact transient-stage cleanup. Never: authority selection, JSON paths, migration, write-fence policy, or result translation.
...
src/cli/state-plane/reset/trace-fs.ts — production SQLite-reset filesystem operations and the ordered observer seam used by the power-cut rig. Never: journal policy, row classification, or reset orchestration.
...
src/cli/reset-journal-inspection.ts — public legacy/SQLite reset inspection result, hook, observation, and halt-error contract. Never: filesystem access, format dispatch, or recovery actions.
src/cli/reset-z-runtime.ts    — shared legacy reset repository-identity/ref prefix observation, recovery-ref creation/normalization, and active-group retirement. Never: journal parsing, artifact paths, or SQLite state mutation.
...
src/cli/reset-journal.ts      — legacy-JSON design-138 journal execution plus authority-format dispatch into the DB-artifact reset facade; legacy v1 halts. Never: SQLite artifact implementation, ordinary BASE composition, consent minting, or P-repair policy.
...
src/cli/doctor-evidence.ts    — what doctor triage may believe: non-mutating workspace/sidecar reads, live-incarnation and boot-binding gates for daemon status, and the inconclusive-vs-proven verdict on remote checks. Never: user-facing wording, remedy selection, or any write.
...
src/cli/binding-registry.ts   — the durable local binding registry (design 211): `~/.rbox/workspaces.json` record/forget/refresh under the shared lockfile + atomic write, the union with daemon desired rows that IS the migration, and stale-root health classification. Never: network calls, workspace mutation, authority over a binding (`.rbox/workspace.json` is), or failing the command it rides along with.
src/cli/doctor-machine.ts     — the all-workspaces view (`status --all`, `doctor --all`, and the outside-a-workspace fallback): enumeration from the binding registry, per-workspace liveness/boot-gated summary, and both renderers. Never: per-workspace check collection, network calls, or mutation.
