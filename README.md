...
- **Dev-aware ignores.** The builtin ignore layer is always enabled, filtering `node_modules`, common build caches, and secrets such as `.env` by default. Interactive wizards also respect `.gitignore` by default; `.rboxignore` supplies shared fleet rules, while folder-config `ignorePaths` supplies machine-local exclusions. A later `!` rule in `.rboxignore` can re-include a specific file (for example `!.env`), or you can opt into syncing Git-ignored files too — still end-to-end encrypted, though large ignored builds and datasets come along as well.
