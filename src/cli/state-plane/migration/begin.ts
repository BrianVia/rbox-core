...
import {
  CONTROL_MAX_BYTES, type AdmissionProof, type HaltResource, type MigrationControl, type SourceWitness,
 ...
 * else, and created exclusively. The sole admitted artifact-ahead image is the
 * exact zero-filled record already there — the same adopt-if-identical rule the
 * control publisher uses for its own revision siblings, and for the same reason:
 * a kill between the write and the fsync must not wedge the phase.
 */
