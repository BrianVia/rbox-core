...
import { readShadowCounters, shadowDoctorLine } from "./sync-git/git-shadow.js";
...
export function renderDoctor(
  ...
  /** #832 shadow-mode summary; omitted until the lane has recorded a pull.
   * Measurement, not a finding — deliberately no status line, nothing to fix. */
  shadowLine?: string,
): string {
...
export async function doctorCmd(root: string, opts: DoctorCmdOptions): Promise<void> {
  ...
  console.log("");
  const shadow = await readShadowCounters(root);
  console.log(renderDoctor(ctx.checks, ctx.localOnly, shadow && shadowDoctorLine(shadow)));
  if (opts.report) {
