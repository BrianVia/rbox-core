...
import {bootstrapAccount,phraseToRk,sha256Hex,utf8} from "../engine/e2ee/index.js";import type {GenesisAccountObservation,GenesisPresence} from "./e2ee-remote.js";import {saveDevice} from "./e2ee-keystore.js";import {genesisPaths,publishGenesisJournal,publishPrepublishMarker,recordGenesisReceipt,stageRecoveryKey,type GenesisJournal} from "./genesis-durable.js";import {classifyEnrollment,hasExactLegacyGenesisPair,type PendingGenesisArtifacts} from "./genesis-enrollment.js";import {genesisQuarantineDir,resumeGenesisQuarantine,startGenesisQuarantine} from "./genesis-quarantine.js";
const ACCOUNT="acct_0123456789abcdef",DEVICE="dev_genesis",ZERO:GenesisPresence={rosters:0,keyStates:0,devices:0,workspaces:0,workspaceKeys:0,e2eePairingTokens:0};let home:string;
beforeEach(async()=>{home=await fs.mkdtemp(path.join(os.tmpdir(),"rbox-genesis-classifier-"));process.env.RBOX_HOME=home;});afterEach(async()=>{delete process.env.RBOX_HOME;await fs.rm(home,{recursive:true,force:true});});
...
describe("design 180 enrollment classifier",()=>{
  ...
  test("no-claim and tombstone local-presence powersets are closed and conservative",async()=>{
    const fields=["stagedRk","device","mk","intentRaw","witnessRaw"] as const;
    for(let mask=0;mask<(1<<fields.length);mask++){
      ...
      fields.forEach((field,index)=>{if(mask&(1<<index)){if(field==="intentRaw"||field==="witnessRaw")pending[field]="present";else pending[field]=true;}});
