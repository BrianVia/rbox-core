...
_rbox_deferrals() {
  ...
  while IFS= read -r row <&\$fd; do
    ...
    case \$reason in
      local-edits|local-index|local-operation|local-commits|local-stash|deletion-pending|conflict|git-busy|stale-unattributed|worktree-ownership|ignored-target|unreadable|artifact|config|containment|unsupported|other) ;;
      *) exec {fd}<&-; return 0 ;;
    esac
