...
type PRepairQRecord = JsonValue | PRepairQ | PRepairQ["p"] | PRepairQ["p"]["payload"]
  | PRepairQ["observed"] | PRepairQ["observed"]["reflog"] | PRepairQ["preserved"]
  | PRepairQ["repair"] | ByteProjection;
type PRepairQJson = PRepairQ & JsonObject;
...
type PRepairReceiptRecord = PRepairQRecord | PRepairReceipt | PRepairReceipt["p"]
  | PRepairReceipt["k"][number] | PRepairReceipt["q"] | PRepairReceipt["origin"]
  | PRepairReceipt["skeep"] | PRepairReceipt["reflog"] | Exclude<PRepairReceipt["eviction"], null>;
...
const primitiveString = (value: JsonValue | undefined): value is string => Object(value) !== value
...
export function skeepHash(oids: readonly string[]) {
...
export interface BuiltPRepairQ { value: PRepairQ; bytes: Uint8Array; ref: string }

export function buildPRepairQ(input: BuildPRepairQInput): BuiltPRepairQ {
  validateInput(input);
...
export function parsePRepairQ(bytes: Uint8Array): PRepairQ {
  ...
  const parsedJson = verifyRoundTrip(Buffer.from(bytes).toString("utf8")) as JsonValue;
  if (parsedJson === null || Array.isArray(parsedJson) || Object.prototype.toString.call(parsedJson) !== "[object Object]") throw new Error("invalid P-repair Q schema");
  const parsed = parsedJson as PRepairQJson;
  if (parsed.v !== 1 || parsed.kind !== "p-repair") throw new Error("invalid P-repair Q schema");
  const exact = (value: PRepairQRecord, keys: readonly string[]): boolean => value !== null
    && Object.prototype.toString.call(value) === "[object Object]"
    && Object.keys(Object(value)).sort().join("\0") === [...keys].sort().join("\0");
  const projection = (value: JsonValue | ByteProjection, cap: number): value is ByteProjection => {
    if (!exact(value, ["bytes", "sha256", "prefixB64", "truncated"])) return false;
    const p = value as ByteProjection;
    if (!safeCounter(p.bytes) || !HEX64.test(p.sha256) || !primitiveString(p.prefixB64)
      || (p.truncated !== true && p.truncated !== false) || p.truncated !== (p.bytes > cap)) return false;
    let prefix: Buffer;
  ...
  if (!HEX64.test(parsed.lineageHash) || !HEX64.test(parsed.repositoryIdentityHash)
    ...
    || !(parsed.observed.incomingKey === null || primitiveString(parsed.observed.incomingKey))
...
export function parsePRepairReceipt(value: JsonValue | PRepairReceipt): PRepairReceipt {
  const keys = (candidate: PRepairReceiptRecord, expected: readonly string[]): boolean => candidate !== null
    && Object.prototype.toString.call(candidate) === "[object Object]"
    && Object.keys(Object(candidate)).sort().join("\0") === [...expected].sort().join("\0");
  ...
  const receipt = value as PRepairReceipt;
  if (receipt.v !== 1 || receipt.kind !== "p-repaired" || !HEX64.test(receipt.lineageHash)
    || !HEX64.test(receipt.repositoryIdentityHash) || !primitiveString(receipt.ref)
    || !HEX32.test(receipt.episode) || !keys(receipt.p, ["ref", "targetOid"])
    || !primitiveString(receipt.p.ref) || !HEX40.test(receipt.p.targetOid)
    || !Array.isArray(receipt.k) || receipt.k.some((entry) => !keys(entry, ["ref", "targetOid"])
      || !primitiveString(entry.ref) || !HEX40.test(entry.targetOid))
    || !keys(receipt.q, ["ref", "targetOid", "value"]) || !primitiveString(receipt.q.ref)
    ...
    || !(receipt.eviction === null || (keys(receipt.eviction, ["qRef", "targetOid"])
      && primitiveString(receipt.eviction.qRef) && HEX40.test(receipt.eviction.targetOid)))) {
