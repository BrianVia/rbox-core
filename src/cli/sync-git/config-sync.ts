...
export function validateCanonicalGitConfig(input: JsonValue): GitConfigValidation {
  if (input === null || Object(input) !== input || Array.isArray(input)) return { ok: false, reason: "config is not an object" };
  const config = input as Readonly<Record<string, JsonValue>>;
  ...
  for (const key of keys) {
    ...
    for (const candidate of values) {
      ...
      if (value === undefined || !gitConfigValueOk(parsed, value)) return { ok: false, reason: `invalid config value: ${key}` };
      if (seen.has(value)) return { ok: false, reason: `duplicate config value: ${key}` };
      seen.add(value);
    }
  }

  const canonical = config as GitConfig;
  const knownRemotes = remoteNames(canonical);
  for (const key of keys) {
    const parsed = parsedByKey.get(key)!;
    if (parsed.section === "branch" && parsed.variable === "remote") {
      for (const value of canonical[key]!) {
        if (!knownRemotes.has(value)) return { ok: false, reason: `branch remote is not in this config: ${value}` };
      }
    }
  }
  if (serializedSize(canonical) > MAX_GIT_CONFIG_SERIALIZED_BYTES) {
