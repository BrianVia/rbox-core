...
function parseCanonical(bytes: Uint8Array): JsonObject {
  const text = Buffer.from(bytes).toString("utf8");
  const parsed = verifyRoundTrip(text) as JsonValue;
  if (parsed === null || Array.isArray(parsed) || Object.prototype.toString.call(parsed) !== "[object Object]") throw new Error("payload is not an object");
...
function parseAbsentPayload(bytes: Uint8Array): BaseAbsentPayload {
  ...
  if (!exactKeys(value, ["lineageHash", "priorOid", "ref", "repositoryIdentityHash", "v"]) || value.v !== 2
    || !jsonString(value.lineageHash) || !HEX64.test(value.lineageHash)
    || !jsonString(value.repositoryIdentityHash) || !HEX64.test(value.repositoryIdentityHash)
    || !jsonString(value.priorOid) || !HEX40.test(value.priorOid)
    || !jsonString(value.ref) || !validBranchRef(value.ref)) throw new Error("invalid A schema");
  return { lineageHash: value.lineageHash, priorOid: value.priorOid, ref: value.ref, repositoryIdentityHash: value.repositoryIdentityHash, v: 2 };
}
...
function parsePresentPayload(bytes: Uint8Array): BasePresentPayload {
  ...
  if (!exactKeys(value, ["episode", "lineageHash", "nextOid", "priorOid", "ref", "repositoryIdentityHash", "v"]) || value.v !== 2
    || !jsonString(value.lineageHash) || !HEX64.test(value.lineageHash)
    || !jsonString(value.repositoryIdentityHash) || !HEX64.test(value.repositoryIdentityHash)
    || !jsonString(value.episode) || !HEX32.test(value.episode)
    || !jsonString(value.nextOid) || !HEX40.test(value.nextOid)
    || !(value.priorOid === null || (jsonString(value.priorOid) && HEX40.test(value.priorOid)))
    || value.priorOid === value.nextOid || !jsonString(value.ref) || !validBranchRef(value.ref)) throw new Error("invalid P schema");
  return { episode: value.episode, lineageHash: value.lineageHash, nextOid: value.nextOid, priorOid: value.priorOid, ref: value.ref, repositoryIdentityHash: value.repositoryIdentityHash, v: 2 };
}
...
function parseSettledMeta(bytes: Uint8Array): SettledAbsenceMeta {
  ...
  if (!exactKeys(value, ["count", "lineageHash", "repositoryIdentityHash", "v"]) || value.v !== 1
    ...
    || !jsonString(value.lineageHash) || !HEX64.test(value.lineageHash)
    || !jsonString(value.repositoryIdentityHash) || !HEX64.test(value.repositoryIdentityHash)) throw new Error("invalid Z metadata schema");
  return { count: Number(value.count), lineageHash: value.lineageHash, repositoryIdentityHash: value.repositoryIdentityHash, v: 1 };
}
