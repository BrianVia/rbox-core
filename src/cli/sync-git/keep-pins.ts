...
export { enumerateRefReflogOids } from "./git-state.js";
...
export function parseKeepPinOrigins(raw: string): KeepPinOrigins {
  ...
  let value: JsonValue;
  try { value = JSON.parse(raw) as JsonValue; } catch { throw new Error("malformed keep-pin origin sidecar"); }
  if (value === null || Array.isArray(value) || Object.prototype.toString.call(value) !== "[object Object]") throw new Error("keep-pin origin sidecar is not an object");
  ...
  for (const [oid, origins] of entries) {
    ...
    parsed[oid] = origins.map((candidate) => {
      if (candidate === null || Array.isArray(candidate) || Object.prototype.toString.call(candidate) !== "[object Object]") throw new Error(`invalid keep-pin origin for ${oid}`);
      ...
      if (Object.prototype.toString.call(record.ref) !== "[object String]" || Object.prototype.toString.call(record.episode) !== "[object String]" || Object.prototype.toString.call(record.time) !== "[object String]") {
        throw new Error(`invalid keep-pin origin strings for ${oid}`);
      }
      const ref = String(record.ref);
      const episode = String(record.episode);
      const time = String(record.time);
      if ([ref, episode, time].some((field) => field.includes("\0") || utf8Bytes(field) > MAX_ORIGIN_STRING_BYTES)) {
      ...
      const identity = `${ref}\0${episode}`;
      if (seen.has(identity)) throw new Error(`duplicate keep-pin origin for ${oid}`);
      seen.add(identity);
      return { ref, episode, time, class: record.class };
    });
  }
  return parsed;
}
...
function mergeOrigin(existing: KeepPinOrigin[], incoming: KeepPinOrigin): KeepPinOrigin[] {
  ...
  if (!same) next.push({ ...incoming });
    ...
    const rank = { tracking: 0, tombstone: 1, human: 2 } satisfies Record<KeepPinOrigin["class"], number>;
