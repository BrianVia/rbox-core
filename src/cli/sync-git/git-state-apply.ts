...
import { receiverEquivalentCollisionNames } from "../../engine/apply-receipt.js";
...
import { HEX40, type RepoCtx, clearIndexResolveUndo, detectGitKind, exists, getGitArtifact, gitBusy, headBranchOf, importGitPackChain, listWorktrees, listWorktreesStrict, moveFileAtomic, repoCtx, warnOnce, ZERO_OID } from "./git-state.js";
...
export async function applyGitState(
  ...
  const eligiblePublishRefs = { ...section.refs } satisfies Record<string, string>;
    ...
    try {
      if (indexTmp) {
        await addTimedMs(opts.chainTimings, "indexOpStateMs", async () => {
          const indexArtifact: GitArtifactRef = {
            sha: section.indexSha!,
            encSha: section.indexEncSha!,
            cipherSize: section.indexCipherSize!,
          };
          if (section.indexComp) {
            indexArtifact.comp = section.indexComp;
            indexArtifact.payloadSha = section.indexPayloadSha;
          }
    ...
    try {
      ...
      for (const [ref, sha] of Object.entries(publishRefs)) {
        ...
        if (ref.startsWith("refs/heads/") && oldOid !== sha && opts.branchTransitions) {
          ...
          try {
            const transitionInput: ApplyBranchTransitionInput = {
              ctx,
              ref,
            ...
            if (expectedReflogFingerprint) transitionInput.expectedReflogFingerprint = expectedReflogFingerprint;
            const committed = await opts.branchTransitions.commit(transitionInput);
