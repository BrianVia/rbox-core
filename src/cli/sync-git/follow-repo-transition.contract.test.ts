...
describe("follow proof minting", () => {
  test("binds the settled standing-branch lineage, repository identity, and incoming key", () => {
    ...
    const proof = followBaseProof(identity, proofReceipt(), advance.progress, true);
  ...
  test("a live stash tip carries stashReflogReady; an absent one does not", () => {
    ...
    const proof = followBaseProof(identity, proofReceipt(), progress({
    ...
    const dropped = followBaseProof(identity, proofReceipt(), progress({
  ...
  test("checkoutComplete is carried verbatim into the locked proof", () => {
    expect(followBaseProof(identity, proofReceipt(), progress(), false).lockedProof.checkoutComplete).toBe(false);
  });

  test("a proof receipt for another repository or wire section is refused", () => {
    expect(() => followBaseProof(identity, proofReceipt({ relPath: "other" }), progress(), true))
      .toThrow(FollowRepoTransitionIdentityMismatch);
    expect(() => followBaseProof(identity, proofReceipt({ incomingKey: "incoming-2" }), progress(), true))
      .toThrow(FollowRepoTransitionIdentityMismatch);
  });
});
