...
import { inTreeWorktreeParentRel, gitSectionBlobRefs, repoCtxFromDisk, type GitRepoKind, type RepoCtx } from "./git-state.js";
import { type OwnedRefMutationBoundary } from "./pins.js";
import { pinDisplaced } from "./keep-pins.js";
...
async function captureAndAuthorizeRepositories(stage: RepoCaptureStage): Promise<void> {
  ...
  for (const rel of [...new Set([...captured, ...carried])].sort()) {
      ...
      if (witness.status === "proven") {
        absentBranchProofs[rel] = witness.proofs;
        continue;
      }
      refuseBranchDeletion(witness.reason, witness.typed);
  }
