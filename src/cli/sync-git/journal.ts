...
import { exists, readRegularFileNoFollow } from "./git-state.js";
...
type CheckoutJournalBindingScalars = Omit<CheckoutJournalBinding, "commonDirIdentity">;
...
interface LegacyCheckoutLockRecord {
...
function scalarBindingsEqual(a: CheckoutJournalBindingScalars, b: CheckoutJournalBinding): boolean {
  return a.stream === b.stream && a.stateNonce === b.stateNonce && a.gitDirReal === b.gitDirReal
    && a.commonDirReal === b.commonDirReal && a.worktreeId === b.worktreeId;
}
...
function isRecord<T>(value: T): value is T & Record<string, T> {
  return value !== null && Object(value) === value && !isCallable(value) && !Array.isArray(value);
}
...
function isBoolean<T>(value: T): value is T & boolean {
  try {
    return Boolean.prototype.valueOf.call(value) === value;
  } catch {
    return false;
  }
}

function validBindingScalars<T>(value: T): value is T & LegacyCheckoutJournalBinding {
  if (!value || !isRecord(value) || Array.isArray(value)) return false;
  const binding = value as Partial<CheckoutJournalBinding>;
  return isString(binding.stream) && isString(binding.stateNonce)
    && isString(binding.gitDirReal) && isString(binding.commonDirReal)
    && isString(binding.worktreeId);
}

function validCheckoutJournalBinding<T>(value: T): value is T & CheckoutJournalBinding {
  if (!validBindingScalars(value)) return false;
  const binding = value as Partial<CheckoutJournalBinding>;
  return !!binding.commonDirIdentity && isRecord(binding.commonDirIdentity)
    && isString(binding.commonDirIdentity.path) && isString(binding.commonDirIdentity.realpath)
    && isString(binding.commonDirIdentity.dev) && isString(binding.commonDirIdentity.ino)
    && isString(binding.commonDirIdentity.birthtimeNs);
}

function validLegacyCheckoutJournalBinding<T>(value: T): value is T & LegacyCheckoutJournalBinding {
  return validBindingScalars(value) && (value as { commonDirIdentity?: unknown }).commonDirIdentity === undefined;
}

function validLockToken<T>(value: T): value is T & { dev: number; ino: number } {
  if (!value || !isRecord(value)) return false;
...
function validCurrentLockToken<T>(value: T): value is T & { dev: number; ino: number; birthtimeNs?: string } {
  if (!value || !isRecord(value)) return false;
  ...
  return Object.keys(token).every((key) => key === "dev" || key === "ino" || key === "birthtimeNs")
    && Number.isSafeInteger(token.dev) && Number(token.dev) >= 0
    && Number.isSafeInteger(token.ino) && Number(token.ino) >= 0
    && (token.birthtimeNs === undefined || (isString(token.birthtimeNs) && /^(?:0|[1-9]\d*)$/.test(token.birthtimeNs)));
}
...
function validExpectedBytes<T>(value: T): value is T & string[] {
  return Array.isArray(value) && value.length > 0 && value.length <= 32 && value.every((encoded) => {
    if (!isString(encoded) || encoded.length > 2048 || !/^(?:[A-Za-z0-9+/]{4})*(?:[A-Za-z0-9+/]{2}==|[A-Za-z0-9+/]{3}=)?$/.test(encoded)) return false;
    const bytes = Buffer.from(encoded, "base64");
    return bytes.length <= 1024 && bytes.toString("base64") === encoded;
  });
}

function validLegacyExpectedBytes<T>(value: T): value is T & string[] {
  if (!Array.isArray(value) || value.length === 0 || value.length > 32) return false;
  let encodedBytes = 0;
  for (const encoded of value) {
    if (!isString(encoded) || encoded.length > 128 * 1024 * 1024
...
function validRefMap<T>(value: T, nullable: boolean): boolean {
  return !!value && isRecord(value) && !Array.isArray(value) && Object.keys(value).length <= 4096
    && Object.entries(value).every(([ref, oid]) => safeRefName(ref)
      && ((nullable && oid === null) || (isString(oid) && VALID_OID.test(oid))));
}

function validBranchInverse<T>(value: T): boolean {
  if (!value || !isRecord(value)) return false;
  const inverse = value as { ref?: unknown; beforeOid?: unknown; afterOid?: unknown; lines?: unknown };
  if (!isString(inverse.ref) || !inverse.ref.startsWith("refs/heads/") || !safeRefName(inverse.ref)
    || (inverse.beforeOid !== null && (!isString(inverse.beforeOid) || !VALID_OID.test(inverse.beforeOid)))
    || (inverse.afterOid !== null && (!isString(inverse.afterOid) || !VALID_OID.test(inverse.afterOid)))
  ...
  for (const raw of inverse.lines) {
    if (!isString(raw) || raw.length > 4096 || raw.includes("\n") || raw.includes("\0")) return false;
    const fields = raw.split(" ") as string[];
...
function validCheckoutJournalCore(journal: CheckoutJournal): boolean {
  if (!isString(journal.incomingKey) || !isBoolean(journal.createdFresh)
    || !journal.old || !isRecord(journal.old) || !isString(journal.old.headContent)
    || !isBoolean(journal.old.indexPresent) || !journal.old.opState || !isRecord(journal.old.opState) || Array.isArray(journal.old.opState)
    || Object.entries(journal.old.opState).some(([rel, present]) => !safeRel(rel) || present !== true)
    || (journal.old.currentRefName !== undefined && (!isString(journal.old.currentRefName) || !safeRefName(journal.old.currentRefName)))
    || (journal.old.currentRefOid !== undefined && (!isString(journal.old.currentRefOid) || !VALID_OID.test(journal.old.currentRefOid)))
    ...
    || !journal.expectedNew || !isRecord(journal.expectedNew)
    || !isString(journal.expectedNew.head) || !journal.expectedNew.opState || !isRecord(journal.expectedNew.opState) || Array.isArray(journal.expectedNew.opState)
    || Object.entries(journal.expectedNew.opState).some(([rel, hash]) => !safeRel(rel) || (hash !== null && !isString(hash)))
...
function validPreparedTransactions(journal: CheckoutJournal, legacy: boolean): boolean {
  ...
  if (transactions !== undefined) {
    ...
    for (const transaction of transactions) {
      if (!transaction || !isRecord(transaction) || (transaction.id !== "primary" && transaction.id !== "post-head")
...
function validCheckoutLockJournal(journal: CheckoutJournal): boolean {
  if (!validCheckoutJournalCore(journal) || !validPreparedTransactions(journal, false)) return false;
  const headLock = journal.expectedNew.headLock;
  if (headLock !== undefined && (!headLock || !isString(headLock.path) || !path.isAbsolute(headLock.path)
    || headLock.path.length > 4096 || !isBoolean(headLock.acquireStarted) || !validExpectedBytes(headLock.expectedBytes)
    || ("token" in headLock && headLock.token !== undefined)
  ...
  if (indexLock !== undefined && (!indexLock || !isRecord(indexLock)
    || !isBoolean(indexLock.acquireStarted)
...
function validLegacyCheckoutLockJournal(journal: CheckoutJournal): boolean {
  if (!validCheckoutJournalCore(journal) || !validPreparedTransactions(journal, true)) return false;
  const legacy = journal.expectedNew as LegacyCheckoutLockRecord;
  const indexLock = legacy.indexLock;
  if (indexLock !== undefined && !validLockToken(indexLock)) return false;
  const headLock = legacy.headLock;
  if (headLock !== undefined && (!headLock || !isString(headLock.path) || !path.isAbsolute(headLock.path)
    || headLock.path.length > 4096 || !isBoolean(headLock.acquireStarted) || !validExpectedBytes(headLock.expectedBytes)
...
async function planAtomicRefRollback(
  ...
  const human: string[] = [];
  const lines: string[] = [];
  type RollbackRefs = Record<string, string | null>;
  const oldRefs: RollbackRefs = { ...(journal.old.refs ?? {}), ...(journal.old.preWipeRefs ?? {}) };
...
async function readLock(abs: string): Promise<import("../../engine/lockfile.js").MarkerObservation | undefined> {
...
interface RecoveryLockCandidate {
  expectedBytes: string[];
  token?: { dev: number; ino: number; birthtimeNs?: string };
  observation?: import("../../engine/lockfile.js").MarkerObservation;
  ownerDead?: boolean;
}
...
async function recoverJournalLocks(
  ...
  const human: string[] = [];
  let liveOwner = false;
  const candidates = new Map<string, RecoveryLockCandidate[]>();
  for (const transaction of journal.expectedNew.preparedTransactions ?? []) {
    ...
    for (const lock of transaction.locks) {
      ...
      const candidate = { expectedBytes: lock.expectedBytes, token: lock.token, ownerDead };
      if (ownerDead === undefined) delete candidate.ownerDead;
      list.push(candidate);
  ...
  if (headLock) {
    ...
    const legacyHead = (journal.expectedNew as LegacyCheckoutLockRecord).headLock;
    const observed = legacy ? undefined : deserializeMarkerObservation(headLock.observation);
    const candidate: RecoveryLockCandidate = {
      expectedBytes: headLock.expectedBytes,
      ownerDead: true,
    };
    if (legacy && legacyHead?.token) candidate.token = legacyHead.token;
    if (observed) candidate.observation = observed;
    list.push(candidate);
...
export async function recoverJournal<T = unknown>(
  ...
  const validId = id === undefined || (isString(id) && /^\d+-[0-9a-f]+$/.test(id));
  const currentSchema = validCheckoutJournalBinding(journal?.binding) && validCheckoutLockJournal(journal);
  const legacySchema = isString(id) && validLegacyCheckoutJournalBinding(journal?.binding)
  ...
  const journalBindingMatches = legacy
    ? scalarBindingsEqual(journal.binding, binding)
