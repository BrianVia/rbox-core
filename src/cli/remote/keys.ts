...
export async function bootstrapKeys(ctx: RemoteContext, body: JsonValue): Promise<void> {
...
/** One field read out of a decoded JSON object: a JSON value, or absent. */
...
const presenceCount=(n:JsonField):number=>{if(typeof n!=="number"||!Number.isSafeInteger(n)||n<0)throw new Error("invalid genesis presence count");return n;};
function parsePresence(value:JsonField):GenesisPresence{
  if(!plain(value)||!keys(value,["rosters","keyStates","devices","workspaces","workspaceKeys","e2eePairingTokens"]))throw new Error("invalid genesis presence");
  return{rosters:presenceCount(value.rosters),keyStates:presenceCount(value.keyStates),devices:presenceCount(value.devices),
    workspaces:presenceCount(value.workspaces),workspaceKeys:presenceCount(value.workspaceKeys),e2eePairingTokens:presenceCount(value.e2eePairingTokens)};
}
...
export async function getGenesisObservation(ctx:RemoteContext):Promise<GenesisAccountObservation>{
  ...
  const claim=decoded as AccountKeysDTO;return{genesisPresenceVersion:1,claim,present,repairTombstone:tomb as GenesisAccountObservation extends {repairTombstone:infer T}?T:never};
}

export async function putDeviceKeys(ctx: RemoteContext, body: JsonValue): Promise<void> {
...
export async function admitDevice(ctx: RemoteContext, body: JsonValue, signal?: AbortSignal): Promise<{ ok: boolean; conflict?: boolean }> {
...
export async function appendRoster(ctx: RemoteContext, body: JsonValue): Promise<{ ok: boolean; conflict?: boolean }> {
