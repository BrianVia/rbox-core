...
const DEFAULT_PULL_JOIN_WATCHDOG_MS = 90_000;
const DEFAULT_PULL_JOIN_WATCHDOG_MAX_FIRINGS = 3;
...
export class BlobBatchDownloader {
  ...
  getToFile(sha: string, expectedSize: number | undefined, destPath: string): Promise<void> {
    return new Promise<void>((resolve, reject) => {
      ...
      const canBatch = !downloadDisabledForProcess && this.config.enabled && expectedSize !== undefined && expectedSize <= this.config.recordBytes;
