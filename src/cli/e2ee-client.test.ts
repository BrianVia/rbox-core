...
test("real pairing handoff excludes real first-marker and repaired-legacy publication through admission",async()=>{
  ...
  for(const kind of ["marker","repaired-legacy"] as const){
    ...
    expect(storedCredentials.state).toBe("valid");
    if(storedCredentials.state!=="valid")throw new Error("pairing did not persist credentials");
    expect(storedCredentials.credentials).toMatchObject({token:"paired-token",deviceId:pairedDeviceId,accountId,remoteUrl:"https://api.test"});
    const storedDevice=await loadDevice(accountId);
    expect(storedDevice&&"secrets" in storedDevice).toBe(true);
    if(!storedDevice||!("secrets" in storedDevice))throw new Error("pairing did not persist E2EE secrets");
    expect(storedDevice.secrets.deviceId).toBe(pairedDeviceId);
    expect(storedDevice.secrets.mk.byteLength).toBeGreaterThan(0);
