...
function isBigInt<T>(value: T): value is T & bigint {
  try {
    return BigInt.prototype.valueOf.call(value) === value;
  } catch {
    return false;
  }
}
...
export function validProcessIncarnation<T>(value: T): value is T & ProcessIncarnation {
  if (!isRecord(value)) return false;
  const owner = value as Partial<ProcessIncarnation>;
  return isString(owner.hostId) && owner.hostId.length > 0 && owner.hostId.length <= 256
    && isString(owner.bootId) && owner.bootId.length > 0 && owner.bootId.length <= 256
    && Number.isSafeInteger(owner.pid) && (owner.pid ?? 0) > 0
    && isString(owner.startTime) && owner.startTime.length > 0 && owner.startTime.length <= 128;
}
...
async function runPsLstart(pid: number): Promise<PsLstartRun> {
  return await new Promise<PsLstartRun>((resolve) => {
    execFile("/bin/ps", ["-p", String(pid), "-o", "lstart="], {
      ...
      const incomplete = !!failure && (isString(failure.code) || failure.killed === true || failure.signal != null);
...
function validBoot<T>(value: T): value is T & HostIdentityBoot {
  if (!isRecord(value)) return false;
...
export function deserializeMarkerObservation<T>(value: T): MarkerObservation | undefined {
  ...
  if ((keys !== "dev,inode,mtimeNs,raw,size" && keys !== "birthtimeNs,dev,inode,mtimeNs,raw,size")
    || !isString(observation.dev) || !UNSIGNED_INTEGER.test(observation.dev)
    || !isString(observation.inode) || !UNSIGNED_INTEGER.test(observation.inode)
    || !isString(observation.size) || !UNSIGNED_INTEGER.test(observation.size)
    || !isString(observation.mtimeNs) || !SIGNED_INTEGER.test(observation.mtimeNs)
    || (observation.birthtimeNs !== undefined && (!isString(observation.birthtimeNs) || !UNSIGNED_INTEGER.test(observation.birthtimeNs)))
    || !isString(observation.raw) || observation.raw.length > MARKER_MAX_BYTES) return undefined;
...
export async function lockStorageLocal(
  ...
  try {
    ...
    const type = isString(stat.type) ? stat.type.toLowerCase() : stat.type;
    ...
    const local = platform === "darwin"
      ? stat.local === true && isString(type) && DARWIN_LOCAL_FS.has(type)
      : platform === "linux" && isBigInt(type) && LINUX_LOCAL_FS.has(type);
...
async function unlinkIfExact(lockPath: string, expected: string | MarkerObservation, hook?: () => void | Promise<void>): Promise<boolean> {
  ...
  if (!before || (isString(expected) ? before.raw !== expected : !sameMarkerObservation(before, expected))) return false;
  await hook?.();
  const after = await readMarkerNoFollow(lockPath);
  if (!after || (isString(expected) ? after.raw !== expected : !sameMarkerObservation(after, expected))) return false;
  try {
