...
function fault(disposition: ConfigFaultDisposition, reason: ConfigFault["reason"], error?: unknown): ConfigFault {
  return { disposition, reason, ...(error === undefined ? {} : { error }) };
}

/** Exhaustive config open/stat/read error classification required by design §4. */
export function classifyConfigFsError(error: unknown): ConfigFault {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    try {
      ...
      try {
        ...
        return { status: "disabled", attempts: attempt, fault: fault("permanent", "lock-unsupported", error) };
      }
      ...
      if (!candidate.ok) {
        lastFault = candidate.fault.disposition === "permanent" ? fault("transient", "candidate-error", candidate.fault.error) : candidate.fault;
        await prepareRetry(lastFault);
        continue;
      }
      ...
      try {
        ...
        warnings.push(`post-rename directory fsync: ${errno(error) ?? String(error)}`);
      }
      ...
      const fsFault = errno(error) ? classifyConfigFsError(error) : undefined;
      lastFault = fsFault?.disposition === "permanent" ? fsFault : fault("transient", "candidate-error", error);
    } finally {
      if (!committed) {
  ...
  for (const name of names) {
    ...
    if (remove) {
      try {
        ...
        if (errno(error) !== "ENOENT") result.spared.push(name);
      }
