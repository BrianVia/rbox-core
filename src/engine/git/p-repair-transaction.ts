...
import { canonicalize } from "../e2ee/jcs.js";
...
import { git, gitRaw, HEX40, ZERO_OID } from "./shared.js";
...
export function reflogObservation(bytes: Uint8Array): { entries: number; top: Buffer | null; oids: string[] } {
...
export async function inspectPRepairReceipt(
  ...
  const pShape: PRepairRetryObservation["p"] = p.kind === "absent" ? "absent"
    : p.kind === "direct" && p.oid === receipt.p.targetOid ? "exact" : "wrong";
  const qShape: PRepairRetryObservation["q"] = q.kind === "absent" ? "absent"
  ...
  const kShape: PRepairRetryObservation["k"] = exactK ? "exact" : absentK ? "absent" : "partial-or-wrong";
  if (qShape === "exact") {
  ...
  const observation: PRepairRetryObservation = {
    receipt: "matching", p: pShape, k: kShape, q: qShape,
