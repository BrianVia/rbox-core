...
import { composeGlobalDelta } from "../../src/cli/sync-state-delta.js";
...
try {
  ...
  // Design 269: the same one-changed save, staged relatively. Its ops come from
  // the composer the real save path uses, so this times the shipped mechanism.
  const deltaFiles = steadyFiles.slice();
  deltaFiles[Math.floor(count / 3)] = file(Math.floor(count / 3), true);
  await timed("save_delta_one_changed", async () => {
    const delta = composeGlobalDelta(steadyFiles, deltaFiles, { nonce: NONCE, stateRevision: steadyRevision });
    if (!delta || delta.ops.length !== 1) throw new Error(`delta composed ${delta?.ops.length ?? "no"} ops`);
    const result = await applySavePacketToStore(store, { ...packet(deltaFiles, 3), globalDelta: delta }, OWNER);
